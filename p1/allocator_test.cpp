...
static bool fillUp(Allocator &a, size_t allocSize, vector<Pointer> &out) {
    int Max =
        (2 * sizeof(buf) / allocSize);  // To avoid creating an infinite loop.
...
TEST(Allocator, DefragAvailable) {
    ...
    try {
        ...
        cerr << "WARNING: Allocator not fragmented initially. Defrag tests are "
                "inconclusive."
             << endl;
    } catch (AllocError &) {
    }
...
TEST(Allocator, ReallocGrow) {
    ...
    if (p.get() == ptr) {
        cerr << "WARNING: Reallocated chunk was not moved. realloc() grow "
                "tests inconclusive."
             << endl;
    }
